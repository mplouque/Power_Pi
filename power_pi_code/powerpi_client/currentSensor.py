...
# Create this datetime object here so it will be preserved through loop iterations
dt = datetime.now()
...
while True:
    ...
    # Grab the running energy usage
    energy = float(0);
    cursor.execute("SELECT wattHours FROM data ORDER BY id DESC LIMIT 1;")
    for result in cursor.fetchall():
        energy = float(result[0])

    # Convert the data to ampere
    current = (msb1 * 65536 + msb * 256 + lsb) / 1000.0
    energy += ((current * float((datetime.now() - dt).seconds)) + (float((datetime.now() - dt).microseconds) / 1000000.0)) / 3600.0
    dt = datetime.now()
    
    # Output data to screen
    #print "Channel no : %d " %(i + 1)
    #print "Current Value : %.3f A" %current
    cursor.execute("INSERT INTO data(amps,watts,wattHours) values({},{},{});".format(current,current*120,energy))
    db.commit()
